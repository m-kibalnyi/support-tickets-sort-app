...
class SupportTicket < ApplicationRecord
  ...
  def process_classification(masker: DataMasker)
    self.filtered_content = masker.mask(raw_content)
    self.category = auto_classify(filtered_content)
    save
  end
  ...
  def auto_classify(content)
    ...
    category = Classifiers::Ai.classify(content) if category == UNCATEGORIZED
    category
  end
end
